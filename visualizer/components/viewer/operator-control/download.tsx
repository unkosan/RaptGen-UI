...
const DownloadCluster: React.FC = () => {
    ...
    const getProbs = (cluster: number) => {
        ...
        const probs = selexData.map((elem) => {
            const x = [elem.coord_x, elem.coord_y]
            const diff = matrix(subtract(x, mean));
            const expMat = multiply(multiply(diff, invcov), transpose(diff)) as unknown as number
    ...
    const handleDownload = async () => {
        ...
        const coord_x = selexData.map((elem) => elem.coord_x);
        const coord_y = selexData.map((elem) => elem.coord_y);
        ...
        if (isProbs) {
            ...
            const clusterProbs = Array.from(Array(length).keys()).map((i) => getProbs(i));
    ...
    return (
        <Form.Group className="mb-3">
            ...
            <InputGroup>
                ...
                {isProbs ? <div></div> : (
                    <InputGroup.Text>
                        <Form.Check label="As Fasta" onChange={(e) => setIsFasta(e.target.checked)} checked={isFasta} />
                    </InputGroup.Text>
                )}
                <Form.Select id="cluster" 
    ...
    return (
        <Form>
            <Form.Group className="mb-3">
                ...
                <Form.Select
                    id="downloadKind"
                    onChange={(e) => setKind(e.currentTarget.value as "cluster" | "encode" | "decode")}
                    value={kind}
                >
                    <option key="cluster" value="cluster">clusters</option>
                    <option key="encode" value="encode">encode sequences</option>
                    <option key="decode" value="decode">decode sequences</option>
                </Form.Select>
            </Form.Group>
