...
const FileUploader: React.FC<FileUploaderProps> = (props) => {
  ...
  const uniquify = (sequences: string[]) => {
    ...
    const duplicates = unique.map(
      (seq) => sequences.filter((s) => s === seq).length
    );
    return {
      unique: unique,
  ...
  const parseSelex = async (file: File) => {
    ...
    if (parsed.invalidCount > 0) {
      setSelexFeedback(
        `File contains ${parsed.invalidCount} invalid sequences.`
      );
    } else {
      setSelexFeedback("");
    }
...
const ParamSelector: React.FC<ParamSelectorProps> = (props) => {
  ...
  const handleForwardAdapterChange = (
  ...
  const handleReverseAdapterChange = (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    const value = e.currentTarget.value.toUpperCase().replace(/T/g, "U");
      ...
      <Form>
        ...
        <Form.Group className="mb-3">
          ...
          <InputGroup hasValidation>
            ...
            />
            <Button onClick={estimateTargetLength} variant="outline-secondary">
              Auto
            </Button>
            <Form.Control.Feedback type="invalid">
              Invalid value
...
const SidebarVaeHome: React.FC<SidebarVaeHomeProps> = (props) => {
  ...
  const [isFileValid, setIsFileValid] = useState(
    Boolean(vaeInput.uniqueSeqs.length)
  );
  ...
  const handleEncode: React.MouseEventHandler<HTMLButtonElement> = async (
    e
  ) => {
    const isModified =
    ...
    if (isModified) {
      ...
      dispatch(
        setVaeNumbers({
          ...
          uniqueEntriesWithAdapters: mask.reduce(
            (a, b) => Number(a) + Number(b),
            0
          ),
        })
      );
      const randomRegions = uniqueSeqs
        .filter((seq, i) => mask[i])
        .map((seq) => {
          return seq.slice(
  ...
  return (
    <>
      ...
      <Button onClick={handleEncode} disabled={!(isParamValid && isFileValid)}>
        Encode
      </Button>
    </>
  );
