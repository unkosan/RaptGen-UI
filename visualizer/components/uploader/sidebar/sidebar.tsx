...
const SidebarHome: React.FC = () => {
  ...
  const measuredConfigInput = useSelector(
    (state: RootState) => state.measuredData.vaeData
  );
  ...
  useEffect(() => {
    const fetchVaeNameList = async () => {
      const res = await axios.get<ResponseVaeModelNames>(
        "/data/VAE-model-names"
      );
  ...
  const handleClickGmm = () => {
    ...
    async function dispatchData() {
      ...
      const resGmmSelex = await axios.get<ResponseSelexData>(
        "/data/selex-data",
        {
          params: {
  ...
  const handleClickMeasured = () => {
    ...
    async function dispatchData() {
      ...
      const resMeasuredSelex = await axios.get<ResponseSelexData>(
        "/data/selex-data",
        {
          params: {
            VAE_model_name: vaeName,
        ...
        <Form.Group className="mb-3">
          <Form.Select
            ...
            onChange={(e) => {
              dispatch(
                setUploadType(
                  e.currentTarget.value as "vae" | "gmm" | "measured"
                )
              );
            }}
          >
            ...
            <option key="gmm" value="gmm">
              GMM
            </option>
            <option key="measured" value="measured">
              Measured List
            </option>
          </Form.Select>
        </Form.Group>
  ...
  return (
    <div className="sidebar">
      ...
      {pseudoRoute === "/vae/submit" ? (
        <SidebarVaeSubmit vaeFile={vaeFile} />
      ) : (
        <></>
      )}
    </div>
